...
export function evaluateDefinition(scope: EvaluationScope, definition: RecipeDefinition,
  ...
  const generation = generateRecipeCandidates({
    ...
    mode: 'shopping_allowed', allocationPolicy: 'expiry_first',
  });
...
export function projectCompositions(input: {
  ...
  for (const slot of ordered) {
    ...
    for (const component of slot.components) {
      ...
      const candidate = evaluateDefinition(input.scope, definition, projectInventoryRows(state), slot.date, slot.servings);
      if (!candidate) {
