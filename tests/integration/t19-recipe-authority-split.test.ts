...
const MODE_CASE_TIMEOUT_MS = 15_000;
...
function planRecipeIds(plan: unknown) {
  ...
  return [
    ...new Set(
      dto.result.meals
        .filter((meal) => meal.source.kind === 'recipe')
        .map((meal) => meal.source.id),
    ),
  ];
}
...
describe('T19 — planner universe must equal Recipe API universe under every authority mode', () => {
  for (const [mode, householdId, expectD1] of [
    ['static', INSIDE, false],
    ['shadow', INSIDE, false],
    ['canary', OUTSIDE, false],
    ['canary', INSIDE, true],
    ['d1', INSIDE, true],
  ] as const) {
    it(
      `${mode}${mode === 'canary' ? (expectD1 ? ' (inside cohort)' : ' (outside cohort)') : ''}: generated plan, alternatives and swap only expose recipes the Recipe API and cooking resolve`,
      async () => {
        ...
        for (const id of planned) {
          expect(
            visible.has(id),
            `planner selected ${id} which GET /recipes does not list under ${mode}`,
          ).toBe(true);
          expect(
            (await call(mode, householdId, 'GET', `/recipes/${id}`)).status,
            `recipe detail ${id}`,
          ).toBe(200);
          expect(
            (await call(mode, householdId, 'POST', `/recipes/${id}/cook/start`)).status,
            `cook start ${id}`,
          ).toBe(200);
        }
        const plan = MealPlanDtoSchema.parse(generated.json);

        const alternatives = await call(
          mode,
          householdId,
          'GET',
          `/meal-planning/plans/${plan.id}/alternatives?revision=${plan.revision}`,
        );
        expect(alternatives.status).toBe(200);
        const offered = PlanAlternativesDtoSchema.parse(alternatives.json).alternatives.map(
          (entry) => entry.id,
        );
        expect(offered.length).toBeGreaterThan(0);
        for (const id of offered)
          expect(visible.has(id), `alternative ${id} is not visible under ${mode}`).toBe(true);
        ...
        const swap = await call(mode, householdId, 'POST', `/meal-planning/plans/${plan.id}/swap`, {
          revision: plan.revision,
          slotId: plan.result.meals[0].slotId,
          replacement: { kind: 'recipe', id: d1Only },
        });
        if (expectD1) {
          ...
          expect(
            (await call(mode, householdId, 'POST', `/recipes/${d1Only}/cook/start`)).status,
          ).toBe(404);
        }
      },
      MODE_CASE_TIMEOUT_MS,
    );
  }
  ...
  it(
    'd1 readiness failure: Recipe API and Planner fall back to static TOGETHER (no split authority)',
    async () => {
      ...
      for (const mode of ['d1', 'canary'] as const) {
        ...
        expect(generated.status, JSON.stringify(generated.json)).toBe(200);
        for (const id of planRecipeIds(generated.json))
          expect(STATIC_IDS.has(id), `${mode} fallback planner selected ${id}`).toBe(true);
        const plan = MealPlanDtoSchema.parse(generated.json);
        const alternatives = await call(
          mode,
          INSIDE,
        ...
        for (const entry of PlanAlternativesDtoSchema.parse(alternatives.json).alternatives)
          expect(STATIC_IDS.has(entry.id)).toBe(true);
        const swap = await call(mode, INSIDE, 'POST', `/meal-planning/plans/${plan.id}/swap`, {
          revision: plan.revision,
          slotId: plan.result.meals[0].slotId,
          replacement: { kind: 'recipe', id: D1_ONLY_COOKABLE },
        });
        expect(swap.status).toBe(422);
        expect(swap.json.code).toBe('REPLACEMENT_NOT_FOUND');
        expect((await call(mode, INSIDE, 'GET', `/recipes/${D1_ONLY_COOKABLE}`)).status).toBe(404);
        expect(
          (await call(mode, INSIDE, 'POST', `/recipes/${D1_ONLY_COOKABLE}/cook/start`)).status,
        ).toBe(404);
      }
    },
    MODE_CASE_TIMEOUT_MS,
  ...
  it(
    'd1 catalog read failure: Recipe API and Planner both serve the static fallback without a 500',
    async () => {
      const batchSizes: number[] = [];
      db.hooks.beforeBatch = (statements) => {
        batchSizes.push(statements.length);
        if (statements.some((statement) => /\bFROM recipes\b/i.test(statement.sql))) {
          throw new Error('D1 recipe catalog unavailable');
        }
      };

      const list = await call('d1', INSIDE, 'GET', '/recipes');
      expect(list.status).toBe(200);
      expect(list.json.recipes).toHaveLength(71);
      ...
      const plan = MealPlanDtoSchema.parse(generated.json);
      for (const id of planRecipeIds(plan)) expect(STATIC_IDS.has(id)).toBe(true);
      expect((await call('d1', INSIDE, 'GET', `/recipes/${D1_ONLY_COOKABLE}`)).status).toBe(404);
      expect(batchSizes.length).toBeGreaterThanOrEqual(4);
    },
    MODE_CASE_TIMEOUT_MS,
  );
  ...
  it('security: no request parameter, header or body field can choose the recipe authority', async () => {
    const spoof = {
      ...
      catalog: 'd1',
      authority: 'd1',
      recipeAuthority: { source: 'd1' },
    };
    const list = await call(
      'static',
      INSIDE,
      'GET',
      `/recipes?RECIPE_CATALOG_MODE=d1&authority=d1&catalog=d1`,
    );
    expect(list.json.recipes).toHaveLength(71);
    const generated = await call('static', INSIDE, 'POST', '/meal-planning/plans', {
      ...intent,
      ...spoof,
    });
    ...
    const swap = await call('static', INSIDE, 'POST', `/meal-planning/plans/${plan.id}/swap`, {
      revision: plan.revision,
      slotId: plan.result.meals[0].slotId,
      replacement: { kind: 'recipe', id: D1_ONLY_COOKABLE },
      ...spoof,
    ...
    expect(
      db.query<{ revision: number }>(
        'SELECT revision FROM generated_meal_plans WHERE id = ?',
        plan.id,
      )[0].revision,
    ).toBe(1);
    // Cross-household: another household cannot read or swap this plan under any mode.
    for (const mode of ['static', 'd1'] as const) {
      expect((await call(mode, OUTSIDE, 'GET', `/meal-planning/plans/${plan.id}`)).status).toBe(
        404,
      );
      expect(
        (
          await call(mode, OUTSIDE, 'POST', `/meal-planning/plans/${plan.id}/swap`, {
            revision: 1,
            slotId: plan.result.meals[0].slotId,
  ...
  it(
    'd1: shopping and cooking accept a D1-only recipe the planner chose',
    async () => {
      const generated = await call('d1', INSIDE, 'POST', '/meal-planning/plans', intent);
      expect(generated.status).toBe(200);
      const plan = MealPlanDtoSchema.parse(generated.json);
      const d1Only = D1_ONLY_COOKABLE;
      const swap = await call('d1', INSIDE, 'POST', `/meal-planning/plans/${plan.id}/swap`, {
        revision: plan.revision,
        slotId: plan.result.meals[0].slotId,
        replacement: { kind: 'recipe', id: d1Only },
      });
      ...
      const shopping = await call(
        ...
        INSIDE,
        'POST',
        `/meal-planning/plans/${plan.id}/shopping`,
        { revision: swapped.revision, currency: 'VND' },
      );
      ...
      const recipe = detail.json.recipe as {
        ingredients: Array<{ ingredientId: string; requiredQuantity: number }>;
        servings: number;
      };
      const deductions = recipe.ingredients.map((line) => ({
        ingredientId: line.ingredientId,
        quantityDeducted: line.requiredQuantity,
      }));
      const eventsBefore = db.query<{ n: number }>(
        'SELECT COUNT(*) AS n FROM inventory_events WHERE household_id = ?',
        INSIDE,
      )[0].n;
      const complete = await call('d1', INSIDE, 'POST', `/recipes/${d1Only}/cook/complete`, {
      ...
      ).toBe(deductions.length);
      expect(
        db.query<{ recipe_id: string }>(
          'SELECT recipe_id FROM cooked_meals WHERE household_id = ?',
          INSIDE,
        ),
      ).toEqual([{ recipe_id: d1Only }]);
      // Static authority for the same household cannot cook that recipe: same 404 the Recipe API gives.
      expect(
        (
          await call('static', INSIDE, 'POST', `/recipes/${d1Only}/cook/complete`, {
            servings: recipe.servings,
            deductions,
          })
        ).status,
